...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="Use intelligent contextual analysis (recommended for complex TSVs).",
)
@click.option(
    "--context-window",
    type=int,
    default=5,
    help="Number of rows to look ahead for context (intelligent mode only, default: 5).",
)
def gazette_refine(
    tsv_file,
    output_path,
    ...
    """
    if intelligent:
        from .gazette.intelligent_refine import intelligent_refine_gazette
        result = intelligent_refine_gazette(
            tsv_path=tsv_file,
            output_path=output_path,
    ...
    click.echo(f"Total tokens: {result['metadata']['total_tokens']}")


@main.command(name="gazette-refinement-verify")
@click.argument("tsv_file", type=click.Path(exists=True, dir_okay=False))
@click.option(
    "-o",
    "--output",
...
def gazette_refinement_verify(
    ...
    if clear_checkpoint:
        from .gazette.verification.checkpoint import VerificationCheckpointManager
    ...
    try:
        from .gazette.verification.engine import LLMVerificationEngine
